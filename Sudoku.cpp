...
void DeleteNumer(int board[N][N], int cantidadAEliminar) {
    int eliminados = 0;
...
void FillRandomBoard(int board[N][N]) {
    memset(board, 0, sizeof(int) * N * N);
    solveSudoku(board, 0, 0);
    DeleteNumer(board, 40); // Ajusta la cantidad de eliminaciones según la dificultad
}
...
void playGame(int board[N][N]){
    ...
    while(true){
        printBoard(board);
        cout << endl << endl;
        cout << "Unable to solve? Enter num to view the solved sudoku."<<endl;
        cout << "Enter row: ";
        cin >> row;
        cout << "Enter column: ";
        cin >> col;
...
int main() {
    ...
    while (true) {
        ...
        int choice;      
        cout << endl << endl;
        cout << "\t\t[0] Solve the test Sudoku" << endl;
